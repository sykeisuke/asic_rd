...
| WS-06 | Extra core supply pair in the `0p5x1` ring | [ ] empirical | Resolve by the padring experiment: modified `slot_0p5x1` ring through `gf180mcu-precheck` and platform COB checks. Public COB rules (fixed bond positions, user-definable nets) suggest feasibility |
